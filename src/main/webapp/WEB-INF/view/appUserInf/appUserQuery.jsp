...
<body>
	<div class="easyui-panel" title=""  style="padding:10px;height:11%">
		<table cellpadding="5">
			<tr>
				<td>手机号码：</td>
				<td><input id="loginName" class="easyui-validatebox textbox"></td>
				<td>真实姓名：</td>
				<td><input id="realName" class="easyui-validatebox textbox"></td>
				<td>激活码：</td>
				<td><input id="activationCode" class="easyui-validatebox textbox"></td>
	...
	<table id="appUserInfDg" class="easyui-datagrid" title="终端用户信息" style="width:auto;height:89%;"
		...
		<thead>
			<tr>
				...
				<th data-options="field:'activationCode',width:220,align:'center'">所用激活码</th>
	...
	<script>
		...
		function delUser(index){  
		    ...
		    if (row){ 
		    	$.messager.confirm('提示', '确定要删除用户['+ row.realName +']吗?', function (r) {
                    if (r) {
                        $.post('${ctx}/appUserInfDel.do', { id: row.id }, function (result) {
		...
		function getData(){
			...
			var cfg = {
					...
					type:"post",
					data:{loginName: loginName,realName: realName,activationCode: activationCode}, 
